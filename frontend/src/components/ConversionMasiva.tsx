...
export default function ConversionMasivaPage({ templateId }: ConversionMasivaPageProps) {
  ...
  // --- LÓGICA DE FASES VISUALES ---
  // Calculamos en qué paso de la maquinaria estamos exactamente
  let currentStep = 0;
  if (phase === 'running') {
    if (!batchProgress) currentStep = 1; // 1. Subiendo archivo pesado a GCS
    else if (batchProgress.total === 0) currentStep = 2; // 2. Desempaquetando y enviando a Tasks
    else currentStep = 3; // 3. Trabajadores renderizando PDFs
  } else if (phase === 'done' && isZipMode) {
    currentStep = 4; // 4. Terminado
  }
  ...
  return (
    <div className="flex flex-col md:h-full md:overflow-hidden">
      <div className="flex flex-1 flex-col gap-5 overflow-auto p-6">
        ...
        {/* Drop zone (Solo se muestra si estamos en IDLE o si hay errores en el modo tradicional) */}
        {phase === 'idle' && (
          <div
      ...
      {/* --- FLUJO MASIVO (ZIP) CON FEEDBACK DE MAQUINARIA --- */}
      {isZipMode && currentStep > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm flex flex-col gap-6">
          
          {/* Timeline / Stepper */}
          <div className="flex items-center justify-between px-2">
             <div className="flex flex-col items-center gap-2 w-1/3">
                <div className={clsx("flex h-10 w-10 items-center justify-center rounded-full border-2 transition-colors", currentStep >= 1 ? "border-primary-500 bg-primary-50 text-primary-600" : "border-gray-200 text-gray-300")}>
                  <CloudUpload size={18} />
                </div>
                <span className={clsx("text-tiny font-semibold text-center", currentStep >= 1 ? "text-primary-700" : "text-gray-400")}>1. Nube GCS</span>
             </div>
             <div className={clsx("h-0.5 w-full -mt-6", currentStep >= 2 ? "bg-primary-500" : "bg-gray-200")} />
             
             <div className="flex flex-col items-center gap-2 w-1/3">
                <div className={clsx("flex h-10 w-10 items-center justify-center rounded-full border-2 transition-colors", currentStep >= 2 ? "border-primary-500 bg-primary-50 text-primary-600" : "border-gray-200 text-gray-300")}>
                  <Server size={18} />
                </div>
                <span className={clsx("text-tiny font-semibold text-center", currentStep >= 2 ? "text-primary-700" : "text-gray-400")}>2. Orquestación</span>
             </div>
             <div className={clsx("h-0.5 w-full -mt-6", currentStep >= 3 ? "bg-primary-500" : "bg-gray-200")} />
             
             <div className="flex flex-col items-center gap-2 w-1/3">
                <div className={clsx("flex h-10 w-10 items-center justify-center rounded-full border-2 transition-colors", currentStep >= 3 ? (currentStep === 4 ? "border-green-500 bg-green-50 text-green-600" : "border-primary-500 bg-primary-50 text-primary-600") : "border-gray-200 text-gray-300")}>
                  {currentStep === 4 ? <CheckCircle2 size={18} /> : <Cpu size={18} />}
                </div>
                <span className={clsx("text-tiny font-semibold text-center", currentStep >= 3 ? (currentStep === 4 ? "text-green-700" : "text-primary-700") : "text-gray-400")}>3. Generación</span>
             </div>
          </div>

          {/* Dinámica según el paso */}
          <div className="rounded-lg bg-gray-50 border border-gray-100 p-4">
            
            {/* Paso 1: Subiendo a GCS */}
            {currentStep === 1 && (
              <div className="flex items-center gap-4 animate-pulse">
                <div className="p-3 bg-white rounded-full shadow-sm"><CloudUpload className="text-primary-500" size={24} /></div>
                <div>
                  <p className="text-sm font-semibold text-gray-800">Transfiriendo archivo pesado a Storage...</p>
                  <p className="text-xs text-gray-500 mt-0.5">La subida depende de tu velocidad de internet. Por favor, no cierres esta pestaña.</p>
                </div>
              </div>
            )}

            {/* Paso 2: Desempaquetando en el servidor */}
            {currentStep === 2 && (
              <div className="flex items-center gap-4 animate-pulse">
                <div className="p-3 bg-white rounded-full shadow-sm"><Server className="text-primary-500" size={24} /></div>
                <div>
                  <p className="text-sm font-semibold text-gray-800">Desempaquetando ZIP en Cloud Run...</p>
                  <p className="text-xs text-gray-500 mt-0.5">Leyendo los XMLs, guardando en Redis y encolando en Cloud Tasks para escalar el trabajo.</p>
                </div>
              </div>
            )}

            {/* Paso 3 y 4: Progreso Paralelo */}
            {currentStep >= 3 && batchProgress && (
               <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Zap className={currentStep === 4 ? "text-green-500" : "text-primary-500 animate-pulse"} size={20} />
                      <h3 className="text-sm font-semibold text-gray-900">
                        {currentStep === 4 ? "Procesamiento Finalizado" : "Workers renderizando PDFs en paralelo"}
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="w-full">
                    ...
                    <div className="h-3 w-full overflow-hidden rounded-full bg-gray-200">
                      <div 
                        className={clsx("h-full transition-all duration-300 rounded-full", currentStep === 4 ? "bg-green-500" : "bg-primary-500")} 
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                    <div className="rounded-lg bg-white p-3 text-center border border-gray-200 shadow-sm">
                    ...
                    <div className="rounded-lg bg-green-50 p-3 text-center border border-green-100 shadow-sm">
                    ...
                    <div className="rounded-lg bg-blue-50 p-3 text-center border border-blue-100 shadow-sm">
                    ...
                    <div className="rounded-lg bg-red-50 p-3 text-center border border-red-100 shadow-sm">
          ...
          {/* Botón único de descarga consolidada (Aparece en Paso 4) */}
          {currentStep === 4 && batchProgress?.status === 'done' && (
            <button
              onClick={handleDownloadBatchZip}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-green-600 py-3 text-sm font-semibold text-white transition-colors hover:bg-green-700 shadow-sm"
            >
              <Download size={18} /> Descargar paquete de PDFs final (.ZIP)
            </button>
          )}
        ...
        {/* Botones de acción (Aparecen si no ha iniciado nada) */}
